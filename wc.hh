...
[[nodiscard]] Count check_mmap_branchless_threads(std::string& file_name) noexcept {
    ...
    std::vector<PaddedBool> starts(THREAD_NUM, {0});
    std::vector<PaddedBool> ends(THREAD_NUM, {0});
    ...
    for (auto i = 0; i < THREAD_NUM; ++i) {
        threads.emplace_back([&, i]() {
            ...
            starts[i].val = is_word[(unsigned char)*ptr];
            ends[i].val = is_word[(unsigned char)*(end - 1)];
    ...
    for (auto i = 1; i < THREAD_NUM; ++i) {
        result.words -= ends[i - 1].val & starts[i].val;
    }
    ...
    for (auto i = 0; i < THREAD_NUM; ++i) {
        threads.emplace_back([&, i]() {
            ...
            for (; ptr + 32 <= end; ptr += 32) {
                ...
                auto is_whitespace = newlines | _mm256_movemask_epi8(_mm256_cmpeq_epi8(data, space)) |
                                     _mm256_movemask_epi8(_mm256_cmpeq_epi8(data, tab)) |
                                     _mm256_movemask_epi8(_mm256_cmpeq_epi8(data, cr));
